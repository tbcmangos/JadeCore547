...
public:
	...
	struct npc_areaguardAI : public Scripted_NoMovementAI
	{
		...
		void MoveInLineOfSight(Unit *pWho)
		{
            ...
            QueryResult result = WorldDatabase.PQuery("SELECT `guard_type`, `trigger_distance`, `req_value`,`tele_map`,`tele_x`,`tele_y`,`tele_z`,`tele_o` FROM `npc_areaguard` WHERE `creature_entry` = %i LIMIT 1", creatureID);
			if (result)
			{
            ...
            case 6:
            {
                // Action based on quest complete status
                if (!pPlayer->GetQuestStatus(reqValue) == QUEST_STATUS_COMPLETE)
                    pPlayer->TeleportTo(teleMap, teleX, teleY, teleZ, teleO);
            }
            break;
			}
