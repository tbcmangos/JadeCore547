...
#define CHECK(exp) \
    if (!(exp))\
    {\
        sLog->outError(LOG_FILTER_GENERAL, "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetGUIDLow(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
