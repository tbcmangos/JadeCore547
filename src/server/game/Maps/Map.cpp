...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = UwowCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = UwowCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = UwowCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<UwowCore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<UwowCore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    UwowCore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<UwowCore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<UwowCore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
...
{
    cell.SetNoCreate();
    UwowCore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<UwowCore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    UwowCore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<UwowCore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<UwowCore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetSpawnId())
    {
        ...
        //bot
            ...
            EnsureGridLoadedForActiveObject(Cell(UwowCore::ComputeCellCoord(c->GetPositionX(), c->GetPositionY())), c);
        //end bot
        else
        {
            GridCoord p2 = UwowCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
...
void Map::RemoveFromActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetSpawnId())
    {
        ...
        //bot
            ...
            EnsureGridLoaded(Cell(UwowCore::ComputeCellCoord(c->GetPositionX(), c->GetPositionY())));
        //end bot
        else
        {
            GridCoord p2 = UwowCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
