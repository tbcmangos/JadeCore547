...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = UwowCore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = UwowCore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = UwowCore::ComputeCellCoord(x + radius, y + radius).normalize();
