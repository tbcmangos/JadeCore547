...
bool Unit::CastSpell(SpellCastTargets const& p_Targets, SpellInfo const* p_SpellInfo, CustomSpellValues const* p_Values, TriggerCastFlags p_TriggerFlags /*= TRIGGERED_NONE*/, Item* p_CastItem /*= nullptr*/, AuraEffect const* p_TriggeredByAura /*= nullptr*/, uint64 p_OriginalCasterGUID /*= 0*/, float p_PeriodicDamageModifier /*= 0.0f*/)
{
    if (!p_SpellInfo)
        return false;
    //npcbot
    if (Creature::IsBotCustomSpell(p_SpellInfo->Id) && !(ToCreature() && ToCreature()->GetBotAI()))
    {
		...
		sLog->outError(LOG_FILTER_UNITS, "CastSpell: NpcBot system custom spell %u by caster: %s %u), aborted. Please report",
			p_SpellInfo->Id, (GetTypeId() == TYPEID_PLAYER ? "player (GUID:" : "creature (Entry:"), (GetTypeId() == TYPEID_PLAYER ? GetGUIDLow() : GetEntry()));
        return false;
    }
    //end npcbot
        ...
        else
        {
            ...
            if (ToCreature()->GetBotAI())
            {
                if (!ToCreature()->CanDodge())
                    return 0.0f;
                dodge = ToCreature()->GetCreatureDodgeChance() + GetTotalAuraModifier(SPELL_AURA_MOD_DODGE_PERCENT);
            }
            //end npcbot
