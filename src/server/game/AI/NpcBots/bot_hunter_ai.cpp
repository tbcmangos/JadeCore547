...
public:
    ...
    struct hunter_botAI : public bot_minion_ai
    {
        ...
        void ApplyClassDamageMultiplierMelee(int32& damage, SpellNonMeleeDamage& damageinfo, SpellInfo const* spellInfo, WeaponAttackType attackType, bool& crit) const
        {
			...
			if (lvl >= 40)
				if (crit && attackType == RANGED_ATTACK) {
					int32 cost_p[MAX_POWERS_COST];
					memset(cost_p, 0, sizeof(uint32) * MAX_POWERS_COST);
					cost_p[MAX_POWERS_COST - 1] = 0;
					m_botSpellInfo->CalcPowerCost(me, m_botSpellInfo->GetSchoolMask(), cost_p);

					(static_cast<ManaRestoreList>(TotH))[spellId] = (int32)cost_p[m_botSpellInfo->GetMainPower()];
				}
				else {
					(static_cast<ManaRestoreList>(TotH))[spellId] = 0;
				}
        }
